...
try {
  ...
  // Threads one level deep, hearts on replies, @mentions with notifications.
  await req(b, "comments/" + p.id, "POST", { body: "Top level from b" }, 201);
  const thTop = (await req(a, "comments/" + p.id)).comments[0];
  assert.equal(thTop.parent_id, null);
  await req(a, "comments/" + p.id, "POST", { body: "right back", parent: thTop.id }, 201);
  const thChild = (await req(b, "comments/" + p.id)).comments.find((c) => c.parent_id === thTop.id);
  assert.ok(thChild, "A reply to a reply nests under it");
  await req(a, "comments/" + p.id, "POST", { body: "@" + handles[1] + " thDeeper still", parent: thChild.id }, 201);
  const thDeeper = (await req(b, "comments/" + p.id)).comments.find((c) => c.body.endsWith("thDeeper still"));
  assert.equal(thDeeper.parent_id, thTop.id, "Threads stay one level deep");
  await req(a, "comments/" + p.id, "POST", { body: "nowhere", parent: "11111111-1111-4111-8111-111111111111" }, 404);
  assert.equal((await req(a, "comment-like/" + thTop.id, "PUT")).likes, 1);
  const thHearted = (await req(a, "comments/" + p.id)).comments.find((c) => c.id === thTop.id);
  assert.equal(thHearted.liked, 1, "The viewer sees their own heart");
  assert.equal(thHearted.likes, 1);
  await req(a, "comment-like/" + thTop.id, "DELETE");
  assert.equal((await req(a, "comments/" + p.id)).comments.find((c) => c.id === thTop.id).likes, 0);
  const thInboxB = (await req(b, "notifications")).items;
  assert.ok(thInboxB.some((n) => n.kind === "comment_reply" && n.actor.handle === handles[0]), "Answering a reply tells its author");
  assert.ok(thInboxB.some((n) => n.kind === "comment_like"), "A heart on a reply tells its author");
  assert.ok(thInboxB.some((n) => n.kind === "mention" && n.post_id === p.id), "An @handle tells the person named");
  await req(b, "comments/" + thTop.id, "DELETE");
  assert.equal((await req(a, "comments/" + p.id)).comments.length, 0, "Removing a reply takes its thread with it");
  pass("Threaded replies, hearts on replies, and @mentions with notifications");
